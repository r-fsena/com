...
import { PropertyType, PresentedProperty, Message, Contact, QuickReplyTemplate } from '@/types/crm';
...
export function WhatsAppInbox() {
  ...
  // Fecha o menu suspenso do canto superior esquerdo ao clicar fora
  React.useEffect(() => {
  ...
  const handleApplyQuickReply = (templateOrText: QuickReplyTemplate | string) => {
    const text = typeof templateOrText === 'string' ? templateOrText : templateOrText.content;
    const clientName = activeContact?.name ? activeContact.name.split(' ')[0] : 'Cliente';
    const brokerName = currentUser.name ? currentUser.name.split(' ')[0] : 'Corretor';
    const tenantName = currentTenant?.name || 'Imobiliária';
    const clientPhone = activeContact?.phone ? formatDisplayPhone(activeContact.phone) : '';

    const customized = text
      .replace(/{{corretor_nome}}|{corretor_nome}|{corretor}/g, brokerName)
      .replace(/{{nome}}|{nome}|{cliente}/g, clientName)
      .replace(/{{imobiliaria}}|{imobiliaria}|{empresa}/g, tenantName)
      .replace(/{{telefone}}|{telefone}/g, clientPhone);

    setMessageInput(customized);

    // Se o modelo contiver imagem, anexa automaticamente pronta para envio
    if (typeof templateOrText !== 'string' && templateOrText.imageUrl) {
      setAttachedMedia({
        type: 'IMAGE',
        url: templateOrText.imageUrl,
        fileName: `${(templateOrText.title || 'modelo').toLowerCase().replace(/[^a-z0-9]/g, '-')}.jpg`,
        fileSize: 150000,
        mimeType: 'image/jpeg',
      });
    }
  ...
  return (
    <div className="flex-1 flex h-full w-full overflow-hidden bg-slate-100">
        ...
        <div className="p-4 border-b border-slate-100 space-y-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5">
              {/* Botão de Modelos de Mensagens */}
              <button
                type="button"
                onClick={() => setIsTemplatesModalOpen(true)}
                className="flex items-center gap-1 bg-amber-50 hover:bg-amber-100 text-amber-800 border border-amber-200/80 text-[10px] font-extrabold px-2.5 py-1.5 rounded-xl transition shadow-2xs cursor-pointer active:scale-95"
                title="Configurar e Gerenciar Modelos de Mensagens"
              >
              ...
              >
                ...
                <span className="hidden sm:inline">Qualificar com IA</span>
              </button>

              {/* Menu Suspenso de Configurações no Canto Superior Esquerdo */}
              <div className="relative" ref={topLeftMenuRef}>
                <button
                  type="button"
                  onClick={() => setShowTopLeftMenu(prev => !prev)}
                  className="p-1.5 text-slate-500 hover:text-slate-900 hover:bg-slate-100 rounded-xl transition cursor-pointer border border-slate-200/70"
                  title="Menu de Configurações e Ações do Inbox"
                >
                  <MoreVertical className="w-3.5 h-3.5" />
                </button>

                {showTopLeftMenu && (
                  <div className="absolute right-0 mt-1.5 w-64 bg-white rounded-2xl shadow-xl border border-slate-200 p-1.5 z-50 animate-in fade-in zoom-in-95 duration-100 divide-y divide-slate-100">
                    ...
                    <div className="py-1 space-y-0.5">
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 hover:bg-amber-50 rounded-xl text-xs font-bold text-amber-900 flex items-center gap-2 transition cursor-pointer"
                      >
                        <div className="w-6 h-6 rounded-lg bg-amber-100 text-amber-700 flex items-center justify-center shrink-0">
                          <Zap className="w-3.5 h-3.5 fill-amber-500" />
                        </div>
                        <div>
                          <p className="leading-tight">Modelos de Mensagens</p>
                          <span className="text-[10px] text-amber-700/80 font-normal">Criar, editar e atalhos com fotos</span>
                        </div>
                      </button>
                      ...
                      <button
                        ...
                        onClick={() => {
                          setShowTopLeftMenu(false);
                          setIsBatchQualifyModalOpen(true);
                        }}
                        className="w-full text-left px-2.5 py-2 hover:bg-indigo-50 rounded-xl text-xs font-bold text-[#3742AC] flex items-center gap-2 transition cursor-pointer"
                      >
                        <div className="w-6 h-6 rounded-lg bg-indigo-100 text-[#3742AC] flex items-center justify-center shrink-0">
                          <Sparkles className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <p className="leading-tight">Qualificar com IA em Massa</p>
                          <span className="text-[10px] text-indigo-700/80 font-normal">Análise automática das conversas</span>
                        </div>
                      </button>
                      ...
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 hover:bg-slate-50 rounded-xl text-xs font-semibold text-slate-700 flex items-center gap-2 transition cursor-pointer disabled:opacity-50"
                      >
                        ...
                        <div>
                          <p className="leading-tight">Sincronizar WhatsApp</p>
                          <span className="text-[10px] text-slate-400 font-normal">Baixar mensagens da Z-API</span>
                        </div>
                      </button>
                    </div>
                    ...
                    <div className="pt-1">
                      <button
                        ...
                        className="w-full text-left px-2.5 py-2 hover:bg-rose-50 rounded-xl text-xs font-bold text-rose-600 flex items-center gap-2 transition cursor-pointer"
                      >
                        <div className="w-6 h-6 rounded-lg bg-rose-100 text-rose-600 flex items-center justify-center shrink-0">
                          <Trash2 className="w-3.5 h-3.5" />
                        </div>
                        <div>
                          <p className="leading-tight">Zerar Base do Inbox</p>
                          <span className="text-[10px] text-rose-500 font-normal">Limpar contatos e histórico</span>
                        </div>
                      </button>
                    </div>
                  </div>
      ...
      <div className="flex-1 flex flex-col h-full bg-[#efeae2] relative min-w-0 overflow-hidden">
        {activeConversation && activeContact ? (
          <>
            ...
            {showQuickReplies && (
              <div className="bg-slate-50 border-t border-slate-200 p-2.5 max-h-60 overflow-y-auto space-y-1.5 shadow-inner">
                <div className="flex items-center justify-between px-2 py-1">
                  ...
                  <button
                    ...
                    className="text-[11px] font-bold text-[#3742AC] hover:underline flex items-center gap-1 cursor-pointer"
                  >
                    <Edit3 className="w-3 h-3" />
                    <span>Gerenciar / Criar Modelos</span>
                  </button>
                </div>

                {quickReplies.filter(qr => qr.isActive !== false).length === 0 ? (
                  <div className="p-3 text-center text-xs text-slate-400 bg-white rounded-xl border border-dashed border-slate-200 space-y-1.5">
                    <p>Nenhum modelo ativo cadastrado.</p>
                    <button
                      type="button"
                      onClick={() => {
                        setShowQuickReplies(false);
                      ...
                      className="inline-block text-xs font-bold text-[#3742AC] hover:underline cursor-pointer"
                    >
                  ...
                  quickReplies
                    .filter(qr => qr.isActive !== false)
                    .map(qr => (
                      <button
                        key={qr.id}
                        onClick={() => handleApplyQuickReply(qr)}
                        className="w-full text-left p-2 hover:bg-indigo-50/80 bg-white border border-slate-200/60 rounded-xl text-xs transition cursor-pointer flex items-center justify-between group shadow-2xs"
                      >
                        <div className="flex items-center gap-2 min-w-0">
                          <span className="font-bold text-[#3742AC] font-mono bg-indigo-50 border border-indigo-150 px-2 py-0.5 rounded-lg text-[11px]">
                            {qr.shortcut}
                          </span>
                        ...
                        <div className="flex items-center gap-1.5 shrink-0 ml-2">
                          {qr.imageUrl && (
                            <span className="text-[10px] text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded-full flex items-center gap-0.5 font-bold">
                              📷 Imagem
                            </span>
                          )}
                          <span className="text-[10px] text-slate-400 group-hover:text-[#3742AC] font-bold">
                            Aplicar ➔
                          </span>
                        </div>
                      </button>
      ...
      {/* Modal de Gerenciamento e Configuração de Modelos de Mensagens */}
      <MessageTemplatesModal
        isOpen={isTemplatesModalOpen}
        onClose={() => setIsTemplatesModalOpen(false)}
        onSelectTemplate={(template) => handleApplyQuickReply(template)}
      />
    </div>
  );
}
