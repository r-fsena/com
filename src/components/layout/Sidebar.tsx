...
export function Sidebar({
  ...
  const isZapiConnected = crmZapiConnected ?? instances.some(i => i.status === 'CONNECTED');
  ...
  const whatsappNavItems = [
    ...
    },
    {
      ...
      icon: QrCode,
      statusDot: isZapiConnected ? 'green' : 'red',
      badge: isZapiConnected ? 'Conectado' : 'Desconectado',
      badgeColor: isZapiConnected 
      ...
      >
        <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
          <div className="relative flex items-center justify-center">
            ...
            {item.statusDot && (
              <span className={`absolute -top-1 -right-1 w-2 h-2 rounded-full ring-2 ${isActive ? 'ring-[#3742AC]' : 'ring-white'} ${item.statusDot === 'green' ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
            )}
          </div>
        ...
        {/* Dot Badge quando Recolhido (para itens normais com badge, se não tiver statusDot) */}
        {isCollapsed && item.badge && !item.statusDot && (
  ...
  return (
    <>
      ...
      {/* Rodapé da Sidebar: Botão Expandir / Recolher Menu Lateral */}
        ...
        {isCollapsed ? (
          <div className="flex justify-center">
            <button
              ...
              onMouseLeave={() => setHoveredTooltip(null)}
              className="p-2.5 rounded-2xl bg-white hover:bg-slate-100 border border-slate-200/80 text-slate-600 hover:text-slate-900 transition cursor-pointer shadow-2xs"
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-2 px-3 rounded-2xl text-xs font-semibold text-slate-500 hover:text-slate-900 bg-white hover:bg-slate-100 transition cursor-pointer border border-slate-200/70 shadow-2xs"
