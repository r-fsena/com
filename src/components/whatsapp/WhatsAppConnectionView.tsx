  ...
  return (
    <div className="flex-1 flex flex-col h-[calc(100vh-5rem)] overflow-hidden bg-[#F0F3FA]">
      ...
      <div className="bg-transparent px-6 sm:px-8 py-5 flex flex-wrap items-center justify-between gap-4 shrink-0">
        ...
        {/* Grupo de Ações do Topo Direito */}
        <div className="flex items-center gap-2.5 flex-wrap">
          {/* Botão de Disparo de Mensagem de Teste */}
          <button
            type="button"
            onClick={() => { setShowTestModal(true); setTestResult(null); }}
            className="bg-white hover:bg-slate-50 text-slate-700 border border-slate-200/90 text-xs font-bold px-4 py-2.5 rounded-full transition shadow-2xs flex items-center gap-2 cursor-pointer active:scale-95"
            title="Abrir formulário de disparo de mensagem de teste"
          >
            <Send className="w-3.5 h-3.5 text-[#3742AC]" />
            <span>Disparo de Teste</span>
          </button>

          {/* Botão de Higienização e Reset da Base */}
          <button
            type="button"
            onClick={() => setShowConfirmReset(true)}
            disabled={isResetting}
            className="bg-rose-50 hover:bg-rose-100 text-rose-700 border border-rose-200/80 text-xs font-bold px-4 py-2.5 rounded-full transition shadow-2xs flex items-center gap-2 cursor-pointer disabled:opacity-50 active:scale-95"
            title="Higienizar e resetar a base de testes"
          >
            <Trash2 className="w-3.5 h-3.5 text-rose-600" />
            <span>{isResetting ? 'Higienizando...' : 'Higienizar Base'}</span>
          </button>
          ...
          <button
            ...
            className="bg-[#3742AC] hover:bg-[#2D368E] text-white text-xs font-bold px-5 py-2.5 rounded-full transition shadow-md shadow-indigo-950/10 flex items-center gap-2 cursor-pointer disabled:opacity-50 active:scale-95"
          >
      ...
      {/* Alerta de Sucesso de Higienização / Reset */}
      {resetSuccess && (
        <div className="mx-6 sm:mx-8 mb-2 bg-emerald-50 text-emerald-800 text-xs font-bold px-4 py-3 rounded-2xl border border-emerald-200 flex items-center gap-2 shadow-2xs animate-in fade-in">
          <Check className="w-4 h-4 text-emerald-600 shrink-0" />
          <span>Base de dados higienizada com sucesso! Leads e conversas resetados.</span>
        </div>
      )}
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2.5 shrink-0 w-full md:w-auto">
              <a
                ...
                download="brokiva-chrome-extension-v1.0.33.zip"
      ...
      {/* Modal de Disparo de Mensagem de Teste */}
      {showTestModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-7 shadow-2xl border border-slate-100 space-y-5 animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-2xl bg-indigo-50 text-[#3742AC] flex items-center justify-center shadow-2xs">
                  <Send className="w-5 h-5" />
                </div>
                <div>
                  <h3 className="text-base font-extrabold text-slate-900">Disparo de Mensagem de Teste</h3>
                  <p className="text-xs text-slate-400">Envie um teste imediato pelo gateway Z-API conectado</p>
                </div>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSendTestMessage} className="space-y-4">
              <div>
                <label className="text-xs font-bold text-slate-700 block mb-1.5">
                  Telefone de Destino (com DDI e DDD):
                </label>
                <input
                  type="text"
                  value={testPhone}
                  onChange={(e) => setTestPhone(e.target.value)}
                  placeholder="Ex: 554888774408"
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-mono text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3742AC]/20 focus:border-[#3742AC] transition"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs text-slate-800 focus:bg-white focus:outline-none focus:ring-2 focus:ring-[#3742AC]/20 focus:border-[#3742AC] transition resize-none leading-relaxed"
                  required
                />
              </div>

              {testResult && (
                <div className={`p-3.5 rounded-2xl border text-xs flex items-center gap-2.5 animate-in fade-in ${
                  testResult.success 
              ...
              <div className="flex items-center justify-end gap-2.5 pt-2 border-t border-slate-100">
                ...
                <button
                  ...
                  className="bg-[#3742AC] hover:bg-[#2D368E] text-white text-xs font-bold px-6 py-2.5 rounded-xl shadow-xs transition flex items-center gap-2 cursor-pointer disabled:opacity-50 active:scale-95"
                >
                  <Send className={`w-3.5 h-3.5 ${isSendingTest ? 'animate-pulse' : ''}`} />
                  <span>{isSendingTest ? 'Disparando...' : 'Enviar Mensagem'}</span>
                </button>
              </div>
            </form>
          </div>
